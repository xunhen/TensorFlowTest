...
class FasterRCNNMetaArch(model.DetectionModel):
    ...
    def predict(self, preprocessed_inputs, true_image_shapes):
        ...
        if self._number_of_stages >= 2:
            ...
            prediction_dict.update(self._predict_second_stage(
                ...
                self._anchors.get() if self.need_rpn else [], image_shape, true_image_shapes))
    ...
    def _postprocess_rpn(self,
        ...
        if self.need_rpn:
            ...
            # change convert and padding to max_num_proposals
            proposal_boxes = self._convert(rpn_box_encodings_batch, image_shapes)
    ...
    #change rpn_box_normalize to rpn_box_absolute
    def _convert(self, rpn_box_encodings_batch, image_shapes):
        # normalize proposal boxes
        def absolute_boxes(args):
            proposal_boxes_per_image = args[0]
            image_shape = args[1]
            absolute_boxes_per_image = box_list_ops.to_absolute_coordinates(
                box_list.BoxList(proposal_boxes_per_image), image_shape[0],
                image_shape[1], check_range=False).get()
            return absolute_boxes_per_image

        absolute_proposal_boxes = shape_utils.static_or_dynamic_map_fn(
            absolute_boxes, elems=[rpn_box_encodings_batch, image_shapes], dtype=tf.float32)
        return absolute_proposal_boxes
    ...
    def provide_rpn_box(self, rpn_boxes_list, rpn_objectness_predictions_with_background):
        self.rpn_boxes_list = rpn_boxes_list
        self.rpn_objectness_predictions_with_background = rpn_objectness_predictions_with_background

    def _get_rpn_box(self):
        return self.rpn_boxes_list, self.rpn_objectness_predictions_with_background
